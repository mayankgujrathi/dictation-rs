...
use super::{
  HISTORY_LEN, UIState, VoiceApp, WINDOW_INNER_SIZE,
};
...
impl VoiceApp {
  ...
  fn update_model_downloading(
    ...
    let progress_raw =
      self.download_progress_atomic.load(Ordering::Relaxed);
    let progress =
      (progress_raw as f32 / 100.0).clamp(0.0, 1.0);
    egui::CentralPanel::default().frame(my_frame).show(
      ctx,
      |ui| {
  ...
  fn update_visualizer_recording(
    ...
    let current_vol =
      self.volume_atomic.load(Ordering::Relaxed) as f32
        / 1000.0;
    ...
    egui::CentralPanel::default().frame(my_frame).show(
      ...
      |ui| {
        ...
        let spacing =
          rect.width() / self.history.len() as f32;
        ...
        for (i, &amp) in self.history.iter().enumerate() {
          ...
          let h =
            (amp * rect.height() * 4.0).clamp(2.0, max_h);
    ...
    egui::CentralPanel::default().frame(my_frame).show(
      ...
      |ui| {
        ui.with_layout(
          ...
          |ui| {
            ui.label(
              egui::RichText::new(text_to_show).color(
    ...
    match status_opt {
      ...
      Some(true) => {
        if self.transcription_rendered_at.is_none() {
          self.transcription_rendered_at =
            Some(Instant::now());
        } else if self
  ...
  fn update(
    ...
    match self.ui_state {
      UIState::ModelDownloading => {
        self.update_model_downloading(ctx, my_frame)
      }
      UIState::VisualizerRecording => {
        self.update_visualizer_recording(ctx, my_frame)
      }
      UIState::Transcribing => {
        self.update_transcribing(ctx, my_frame)
      }
    }
  }
}
