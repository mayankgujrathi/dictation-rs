...
fn run_model_download(progress: Arc<AtomicU32>) {
  ...
  let endpoint_list: Vec<(&str, String)> =
    endpoints.collect();
  ...
  for (filename, url) in endpoint_list {
    let mut response = match client.get(url).send() {
      ...
      Err(e) => {
        eprintln!(
          "Download request failed for {filename}: {e}"
        );
        return;
      }
    };
    ...
    let mut file =
      match std::fs::File::create(model_dir.join(filename))
      {
        Ok(f) => f,
        Err(_) => return,
      };
    ...
    loop {
      let read = match response.read(&mut buffer) {
        ...
        Err(e) => {
          eprintln!(
            "Download stream read failed for {filename}: {e}"
          );
          return;
        }
      };
      ...
      downloaded_bytes =
        downloaded_bytes.saturating_add(read as u64);
      if total_bytes > 0 {
        let pct = ((downloaded_bytes.saturating_mul(100))
          / total_bytes)
          .min(100) as u32;
  ...
  let _ = std::fs::write(
    model_success_flag_path(),
    b"downloaded",
  );
}

fn run_transcription(
  status_slot: Arc<Mutex<Option<bool>>>,
) {
...
impl VoiceApp {
  pub(crate) fn spawn_model_download_worker_if_needed(
    &mut self,
  ) {
