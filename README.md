...
3. Press `Ctrl` (Windows/Linux) or `Command` (macOS) to start recording.

4. Press the same key again to stop and save the audio.
...
- **rdev**: Global keyboard listener for modifier-key toggle (`Ctrl` on Windows/Linux, `Command` on macOS)
